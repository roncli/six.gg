...
class Twitch {
    ...
    static async connect() {
        ...
        return Boolean(Twitch.#channelAuthProvider && Twitch.#botAuthProvider);
    }
    ...
    static async login() {
        ...
        Twitch.#channelAuthProvider.onRefresh(async (_userId, token) => {
        ...
        Twitch.#botAuthProvider.onRefresh(async (_userId, token) => {
    ...
    static logout() {
        try {
            Twitch.#channelChatClient.client.quit();
        } catch { } // eslint-disable-line no-empty -- If it fails to quit, just fail silently.
        Twitch.#channelChatClient = void 0;

        try {
            Twitch.#botChatClient.client.quit();
        } catch { } // eslint-disable-line no-empty -- If it fails to quit, just fail silently.
    ...
    static async setStreamInfo(title, game) {
        ...
        try {
            ...
            gameId = gameData.id;
        } catch { } // eslint-disable-line no-empty -- If it fails to get the game ID, just fail silently.
    ...
    static setupChat() {
        ...
        Twitch.#channelChatClient.client.onDisconnect((_manually, reason) => {
        ...
        Twitch.#channelChatClient.client.onPrimeCommunityGift((channel, _user, subInfo) => {
        ...
        Twitch.#botChatClient.client.onDisconnect((_manually, reason) => {
            if (!reason || reason.message === "[1006] ") {
                // Ignore network disconnects, or disconnects without a reason.
                return;
            }
