    ...
    async remove(member) {
        ...
        if (member.id === this.featured) {
            ...
            if (this.streamers.size === 0) {
                ...
                const [data] = Array.from(this.streamers.keys()).map((s) => ({id: s, twitchName: this.streamers.get(s).twitchName, index: this.previouslyFeatured.indexOf(s)})).sort((a, b) => a.index - b.index);
