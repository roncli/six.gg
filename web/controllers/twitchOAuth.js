 ...
 */
class TwitchOAuth extends HotRouter.RouterBase {
    ...
    static async get(req, res) {
        ...
        if (scope.filter((/** @type {string} */ s) => process.env.TWITCH_CHANNEL_SCOPES.split(" ").indexOf(s) === -1).length === 0 && process.env.TWITCH_CHANNEL_SCOPES.split(" ").filter((s) => scope.indexOf(s) === -1).length === 0) {
            tokens.channelAccessToken = accessToken;
            tokens.channelRefreshToken = refreshToken;
        } else if (scope.filter((/** @type {string} */ s) => process.env.TWITCH_BOT_SCOPES.split(" ").indexOf(s) === -1).length === 0 && process.env.TWITCH_BOT_SCOPES.split(" ").filter((s) => scope.indexOf(s) === -1).length === 0) {
