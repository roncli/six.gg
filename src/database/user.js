...
class UserDb {
    ...
    static async getByEventAttendees(eventId) {
        ...
        const attendees = /** @type {UserTypes.UserMongoData[]} */(await db.collection("attendee").aggregate([
            {
                $match: {
    ...
    static async getBySession(id, ip) {
        ...
        const encryptedData = /** @type {{user: UserTypes.UserMongoData, session: SessionTypes.EncryptedMongoSessionData}} */(await db.collection("session").aggregate([
            {
                $match: {
