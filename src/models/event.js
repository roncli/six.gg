...
class Event {
    ...
    static async notifyEventStarting(id) {
        ...
        await attendees.reduce((promise, attendee) => promise.then(async () => {
            try {
                const user = await Discord.findUserById(attendee.discordId);
                await Discord.queue(`**${Encoding.discordEncode(event.title)}** begins in 30 minutes!`, user.dmChannel);
            } catch (err) {
