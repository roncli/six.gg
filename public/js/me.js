...
class Me {
    ...
    static async #saveLocation(ev) {
        const input = /** @type {HTMLInputElement} */(ev.target),
            span = /** @type {HTMLSpanElement} */(input.nextSibling);
    ...
    static async #saveTimezone(ev) {
        const select = /** @type {HTMLSelectElement} */(ev.target),
            span = /** @type {HTMLSpanElement} */(select.nextSibling);
    ...
    static DOMContentLoaded() {
        ...
        if (location) {
            location.addEventListener("focusout", Me.#saveLocation);
        }

        if (timezone) {
            timezone.addEventListener("change", Me.#saveTimezone);
        }
    }
}
